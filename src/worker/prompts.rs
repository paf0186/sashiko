...
```"#;
            ...
            while attempts < max_attempts && !success {
                    ...
                    .await
                {
                    ...
                    Err(e) => {
                        // Fail fast for structural errors — retrying would just
                        // hit the same limit again and waste tokens/turns.
        ...
        let mut findings_json = Value::Array(Vec::new());
        {
            ...
            let user_prompt = format!(
                "{}\n\nCRITICAL REVIEW DIRECTIVE: To dismiss a concern as a false positive, you must find concrete evidence in the code that proves the concern is invalid (e.g., verifying the caller handles the edge case). If you cannot find concrete proof of safety, you must retain the concern.\n\nFull Series Context:\n{}\n\nAggregated Concerns:\n{}\n\nFor each finding you retain, you MUST determine the exact file path and line number by using the search_file_content or read_files tools to locate the relevant code. Do not guess or leave these null — use tools to find the precise location before emitting the finding.\n\nReturn ONLY a JSON object with a 'findings' array. Each object in the 'findings' array MUST use exactly the following keys: \"problem\" (a string containing the vulnerability description), \"severity\" (a string: Low, Medium, High, or Critical), \"severity_explanation\" (a string detailing the reasoning and proof), \"file_path\" (the path of the affected file relative to the repository root — REQUIRED, must not be null), \"line_number\" (the line number in the patched file where the issue occurs — REQUIRED, must not be null).\n\nExample Output:\n```json\n{{\n  \"findings\": [\n    {{\n      \"problem\": \"Memory leak in function X when condition Y is met.\",\n      \"severity\": \"High\",\n      \"severity_explanation\": \"1. Condition Y is met.\\\n2. The buffer is allocated but not freed before return.\",\n      \"file_path\": \"fs/lustre/llite/file.c\",\n      \"line_number\": 1234\n    }}\n  ]\n}}\n```",
