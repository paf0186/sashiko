...
impl AiProvider for GeminiCliProvider {
    async fn generate_content(&self, request: AiRequest) -> Result<AiResponse> {
        ...
        // gemini -p "process this" -o json --sandbox -m MODEL
        // The big prompt goes via stdin to avoid ARG_MAX issues (sashiko
        // prompts can exceed 400KB). A short -p flag triggers non-interactive
        // mode; the actual content is appended from stdin per gemini docs.
        // --sandbox runs in a Docker container (requires Docker).
        let mut child = Command::new("gemini")
            .args([
                "-p", "Process the following review request.",
                "-o", "json",
                "--sandbox",
                "-m", &self.model,
            ])
        ...
        if !output.status.success() {
            ...
            warn!("gemini CLI failed. exit={}, stdout_len={}, stderr_len={}", output.status, stdout.len(), stderr.len());
            warn!("gemini-cli stdout: {}", &stdout[..stdout.len().min(500)]);
            warn!("gemini-cli stderr: {}", &stderr[..stderr.len().min(500)]);
            anyhow::bail!("gemini CLI exited with {}: {}", output.status, stderr.trim());
        }

        let raw = String::from_utf8_lossy(&output.stdout);
        debug!("gemini-cli raw output ({} bytes): {}", raw.len(), &raw[..raw.len().min(200)]);
